...
filter:
	python scripts/find_low_alpha.py $(base_dir) > best_models/optimization_results_20260320_093755.txt
	python scripts/clean_models.py best_models/optimization_results_20260320_093755.txt > best_models/optimization_results_20260320_093755.json

# Get global minimum score
find-min-score-all:
	@echo "Minimum score across all optimization results:"
	@grep -oP '"score": \K[0-9.e-]+' best_models/optimization_results_20260320_093755.txt | \
		sort -n | \
		head -n 1

# Find details for the global minimum score
find-min-details:
	$(eval MIN_SCORE := $(shell grep -oP '"score": \K[0-9.e-]+' best_models/optimization_results_20260320_093755.txt | sort -n | head -n 1))
	$(eval RAW_ALPHA := $(shell awk -v score="$(MIN_SCORE)" 'BEGIN{RS=""; FS="\n"} $$0 ~ score {for(i=1;i<=NF;i++) if($$i ~ /_raw_alpha/) {split($$i,a,":"); gsub(/[ ,}]/, "", a[2]); print a[2]; exit}}' best_models/optimization_results_20260320_093755.txt))
	$(eval FILE_PATH := $(shell awk -v score="$(MIN_SCORE)" 'BEGIN{RS=""; FS="\n"} $$0 ~ score {for(i=1;i<=NF;i++) if($$i ~ /^File:/) {gsub(/^File: /, "", $$i); print $$i; exit}}' best_models/optimization_results_20260320_093755.txt))
